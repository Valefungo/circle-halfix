...
int pc_init(struct pc_settings* pc)
{
    ...
    LOG("PC", "pc_init component start\n");
    ...
    LOG("PC", "pc_init cpu_set_a20 start\n");

    //cpu_set_a20(0); // causes code to be prefetched from 0xFFEFxxxx at boot
    cpu_set_a20(1);

    LOG("PC", "pc_init io_trigger_reset start\n");

    io_trigger_reset();

    LOG("PC", "pc_init display_init start\n");

    display_init();

    LOG("PC", "pc_init register start\n");
